...
<center><h1>Incoming Orders - Kitchen </h1></center>
...
$table_query = mysqli_query($conn, "SELECT distinct table_number from kitchen_preview ORDER BY id DESC");
$table_numbers = mysqli_num_rows($table_query);

for ($u=0; $u < $table_numbers; $u++) { 
	$table_info = mysqli_fetch_array($table_query); 
	 $order_query = mysqli_query($conn, "SELECT distinct `order` from kitchen_preview where table_number = '$table_info[0]' ORDER BY id desc");
	 $order_numbers = mysqli_num_rows($order_query);
	 $order_query1 = mysqli_query($conn, "SELECT * from kitchen_preview where table_number = '$table_info[0]' ORDER BY id DESC");
   ...
   $order_query = mysqli_query($conn,"SELECT distinct `order` from kitchen_preview where table_number = '$table_info[0]' ORDER BY id DESC ");
    ...
    $order_info = mysqli_fetch_array($order_query);
     $check1 = mysqli_query($conn,"SELECT SUM(quantity) from kitchen_preview where `order` = '$order_info[0]' and table_number = '$table_info[0]' ");
...
if(isset($_POST['complete'])){
	$tab = $_POST['table_num'];
	mysqli_query($conn,"DELETE from kitchen_preview where table_number = $tab");
